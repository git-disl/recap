...
from transformers.modeling_outputs import BaseModelOutput, BaseModelOutputWithCrossAttentions, SemanticSegmenterOutput
from transformers.modeling_utils import PreTrainedModel
from transformers.utils import logging
from transformers.models.mask2former.configuration_mask2former import Mask2FormerConfig

from utils import get_confusion_matrix, process_segmenter_output
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def forward(
        ...
        if mask_labels is None:
            mask_labels = torch.ones((pixel_values.shape[0], pixel_values.shape[2], pixel_values.shape[3]), device=pixel_values.device)
        ...
        if labels is not None:
            ...
            loss = self.get_loss(loss_dict)[0]
        ...
        output = Mask2FormerForUniversalSegmentationOutput(
            loss=loss,
            class_queries_logits=class_queries_logits[-1].float(),  # 1
            masks_queries_logits=masks_queries_logits[-1],  # 2
            auxiliary_logits=auxiliary_logits,
            encoder_last_hidden_state=outputs.encoder_last_hidden_state,  # 3
            pixel_decoder_last_hidden_state=outputs.pixel_decoder_last_hidden_state.float(),
        ...
        logits = torch.stack(process_segmenter_output(output, target_sizes=[pixel_values[i].shape[-2:] for i in range(len(pixel_values))]), dim=0)
        index_labels = torch.stack([torch.argmax(mask_labels[i], dim=0) - (mask_labels[i].sum(dim=0) == 0).float() for i in range(len(labels))], dim=0)
        index_labels_ = torch.ones_like(index_labels.long()) * 255
        for i in range(len(index_labels_)):
            for j, l in enumerate(labels[i]):
                index_labels_[i][index_labels[i] == j] = l

        confusion_matrix = get_confusion_matrix(index_labels_.detach().cpu().numpy(), logits.detach().cpu().numpy(), pixel_values.shape[-2:], 19, 255)
        confusion_matrix = torch.tensor(confusion_matrix).unsqueeze(0).int()

        return SemanticSegmenterOutput(
            loss=loss,
            logits=confusion_matrix,
            hidden_states=None,
            attentions=None,
        )
